...
Процедура СоздатьРасширение(
    ...
    Ожидаем.Что(СравнитьВерсии(Версия, "8.3.20") > 0, "Версия платформы 1С должна быть старше, чем 8.3.20").ЭтоИстина();
    Ожидаем.Что(ЯвляетсяИменем(ИмяРасширения), "Параметром должна быть строка, удовлетворяющая требованиям к имени переменной").ЭтоИстина();
    Ожидаем.Что(ЯвляетсяИменем(ПрефиксИмен), "Параметром должна быть строка, удовлетворяющая требованиям к имени переменной").ЭтоИстина();
    ...
    ВыполнитьКоманду(102);  // это костыль, но команда падает с таким кодом. При этом расширение создается.
КонецПроцедуры  
...
Процедура ИзменитьСвойстваРасширения(
    ...
    Ожидаем.Что(СравнитьВерсии(Версия, "8.3.20") > 0, "Версия платформы 1С должна быть старше, чем 8.3.20").ЭтоИстина();
    Ожидаем.Что(ЯвляетсяИменем(ИмяРасширения), "Параметром должна быть строка, удовлетворяющая требованиям к имени переменной").ЭтоИстина();
    Ожидаем.Что(Активность).ИмеетТип("Булево");
    Ожидаем.Что(БезопасныйРежим).ИмеетТип("Булево");
    Ожидаем.Что(ИмяПрофиляБезопасности).ИмеетТип("Булево");
    Ожидаем.Что(ЗащитаОтОпасныхДействий).ИмеетТип("Булево");
    Ожидаем.Что(ИспользуетсяВ_РИБ).ИмеетТип("Булево");
...
Процедура УстановитьПараметрыАвторизацииИБ(
    ...
    Если СравнитьВерсии(Версия, "8.3.18.1563") > 0 Тогда
        ПараметрыАвторизацииИБ.Добавить("--user=" + ПользовательИБ);
        Если Не (ПарольИБ = Неопределено Или ПарольИБ = "") Тогда
            ПараметрыАвторизацииИБ.Добавить("--password=" + ПарольИБ);
        КонецЕсли;    
    КонецЕсли;
...
Функция ВыполнитьКоманду(УстановленныйКодВозврата = 0)
	...
	Если КодВозврата <> УстановленныйКодВозврата Тогда
...
//  ВАрхив - Булево - выгружать в zip-архив XML-файлов. Параметр КаталогВыгрузки должен быть именем файла.
